...
/**
 * Service for retrieving product data.
 */
class PimCsvProcessor {
  ...
  /**
   * Constructs a new ProductDataService object.
   *
   ...
   */
  public function parseCsvFile($fileUri) {
    $csv_rows = [];
  ...
  /**
   * Imports entities from a CSV file into Drupal.
   */
  public function setNodeFieldValues(Node $node, array $mapped_data) {
    ...
    if (isset($mapped_data['category.product_category'])) {
      if ($mapped_data['category.product_category'] !== '' && $mapped_data['category.product_category'] !== 'NULL') {
        ...
        $vocabulary = $this->entityTypeManager->getStorage('taxonomy_vocabulary')->load($vocabulary_machine_name);
    ...
    if (isset($mapped_data['product_attributes.quantity'])) {
      ...
      elseif ($mapped_data['product_attributes.quantity'] == 'NULL') {
        ...
        $quantity = $existing_quantity_value ?? NULL;
      }
    }
    ...
    if (isset($mapped_data['product_attributes.color'])) {
      ...
      elseif ($mapped_data['product_attributes.color'] == 'NULL') {
        ...
        $color = $existing_color_value ?? NULL;
      }
    }
    ...
    if (isset($mapped_data['product_attributes.weight'])) {
      ...
      elseif ($mapped_data['product_attributes.weight'] == 'NULL') {
        ...
        $weight = $existing_weight_value ?? NULL;
      }
    }
    ...
    if (isset($mapped_data['marketing.sales_copy'])) {
      ...
      elseif ($mapped_data['marketing.sales_copy'] == 'NULL') {
        ...
        $sales_copy = $existing_sales_copy ?? NULL;
      }
    }
    ...
    if (isset($mapped_data['marketing.social_media_content'])) {
      ...
      elseif ($mapped_data['marketing.social_media_content'] == 'NULL') {
        ...
        $social_media_content = $existing_social_media_content ?? NULL;
      }
    }
    ...
    if (isset($mapped_data['marketing.user_guides'])) {
      ...
      elseif ($mapped_data['marketing.user_guides'] == 'NULL') {
        ...
        $user_guides = $existing_user_guide ?? NULL;
      }
    }
    ...
    if (isset($mapped_data['assets.image'])) {
      ...
      elseif ($mapped_data['assets.image'] == 'NULL') {
        ...
        $asset_image = $existing_asset_image ?? NULL;
      }
    }
    ...
    if (isset($mapped_data['assets.video'])) {
      ...
      elseif ($mapped_data['assets.video'] == 'NULL') {
        ...
        $asset_video = $existing_asset_video ?? NULL;
      }
    }
    ...
    if (isset($mapped_data['assets.audio'])) {
      ...
      elseif ($mapped_data['assets.audio'] == 'NULL') {
        ...
        $asset_audio = $existing_asset_audio ?? NULL;
      }
    }
    ...
    if (isset($mapped_data['assets.pdf'])) {
      ...
      elseif ($mapped_data['assets.pdf'] == 'NULL') {
        ...
        $asset_pdf = $existing_asset_pdf ?? NULL;
      }
    }
    ...
    $prod_grp_model = $this->getParagraphFieldValue($mapped_data['prod_grp.model'], $existing_prd_model ?? NULL);
    // Process 'prod_grp.part_number' column.
    $prod_grp_part_number = $this->getParagraphFieldValue($mapped_data['prod_grp.part_number'], $existing_prd_part_number ?? NULL);
    // Process 'prod_grp.description' column.
    $prod_grp_description = $this->getParagraphFieldValue($mapped_data['prod_grp.description'], $existing_prd_description ?? NULL);
    // Process 'prod_grp.marketing_description' column.
    $prod_grp_marketing_description = $this->getParagraphFieldValue($mapped_data['prod_grp.marketing_description'], $existing_marketing_description ?? NULL);
    ...
    $product_conformal_coating = $this->getParagraphFieldValue($mapped_data['prod_grp.conformal_coating'], $existing_prd_conformal_coating ?? NULL);
    // Process 'prod_grp.hdmi' column.
    $product_hdmi = $this->getParagraphFieldValue($mapped_data['prod_grp.hdmi'], $existing_prd_hdmi ?? NULL);
    // Process 'prod_grp.reliability_mtbf' column.
    $product_reliability_mtbf = $this->getParagraphFieldValue($mapped_data['prod_grp.reliability_mtbf'], $existing_prd_reliability_mtbf ?? NULL);
    ...
    if (isset($mapped_data['prod_grp.upc'])) {
      ...
      elseif ($mapped_data['prod_grp.upc'] == 'NULL') {
        ...
        $product_upc = $existing_prd_upc ?? NULL;
      }
    }
    // Process 'prod_grp.eccn' column.
    $product_eccn = $this->getParagraphFieldValue($mapped_data['prod_grp.eccn'], $existing_prd_eccn ?? NULL);
    // Process 'prod_grp.series' column.
    $product_series = $this->setParagraphTaxonomyField('product_series', $mapped_data['prod_grp.series'], $existing_prd_series ?? NULL);
    // Process 'prod_grp.family' column.
    $product_family = $this->setParagraphTaxonomyField('product_family', $mapped_data['prod_grp.family'], $existing_prd_family ?? NULL);
    ...
    $ports_copper = $this->setFieldWithAllowedValues($mapped_data, 'ports_group.copper', $copper_allowed_values, $existing_ports_copper ?? NULL);
    // Process 'ports_group.ethernet' column.
    $ports_ethernet = $this->getParagraphFieldValue($mapped_data['ports_group.ethernet'], $existing_ports_ethernet ?? NULL);
    // Process 'ports_group.fiber' column.
    $ports_fiber = $this->getParagraphFieldValue($mapped_data['ports_group.fiber'], $existing_ports_fiber ?? NULL);
    // Process 'ports_group.serial_ports' column.
    $serial_ports_allowed_values = ['rs232', 'rs485', 'sw_defined'];
    $ports_serial = $this->setFieldWithAllowedValues($mapped_data, 'ports_group.serial_ports', $serial_ports_allowed_values, $existing_ports_serial ?? NULL);
    // Process 'ports_group.usb' column.
    $ports_usb = $this->getParagraphFieldValue($mapped_data['ports_group.usb'], $existing_ports_usb ?? NULL);
    ...
    $cert_ul = $this->getParagraphFieldValue($mapped_data['cert.ul'], $existing_cert_ul ?? NULL);
    // Process 'cert.fc' column.
    $cert_fc = $this->getParagraphFieldValue($mapped_data['cert.fc'], $existing_cert_fc ?? NULL);
    // Process 'cert.ce' column.
    $cert_ce = $this->getParagraphFieldValue($mapped_data['cert.ce'], $existing_cert_ce ?? NULL);
    // Process 'cert.ukca' column.
    $cert_ukca = $this->getParagraphFieldValue($mapped_data['cert.ukca'], $existing_cert_ukca ?? NULL);
    // Process 'cert.ukca' column.
    $cert_ex = $this->getParagraphFieldValue($mapped_data['cert.ex'], $existing_cert_ex ?? NULL);
    // Process 'cert.dnv' column.
    $cert_dnv = $this->getParagraphFieldValue($mapped_data['cert.dnv'], $existing_cert_dnv ?? NULL);
    // Process 'cert.shock_vibration' column.
    $shock_vibration_allowed_values = ['iec_68_2_6', 'iec_68_2_27'];
    $cert_shock_and_vibration = $this->setFieldWithAllowedValues($mapped_data, 'cert.shock_vibration', $shock_vibration_allowed_values, $existing_cert_shock_vibration ?? NULL);
    ...
    $mech_enclosure = $this->setFieldWithAllowedValues($mapped_data, 'mech.enclosure', $mechanical_enclosure_allowed_values, $existing_mech_enclosure ?? NULL);
    // Process 'mechmounting' column.
    $mechanical_mounting_allowed_values = [
      'din_rail', 'rack', 'panel_mount',
    ];
    $mech_mounting = $this->setFieldWithAllowedValues($mapped_data, 'mech.mounting', $mechanical_mounting_allowed_values, $existing_mech_mounting ?? NULL);
    // Process 'mech.abs' column.
    $mech_abs = $this->getParagraphFieldValue($mapped_data['mech.abs'], $existing_mech_abs ?? NULL);
    // Process 'mech.front_wiring' column.
    $mech_front_wiring = $this->getParagraphFieldValue($mapped_data['mech.front_wiring'], $existing_mech_front_wiring ?? NULL);
    // Process 'mech.top_wiring' column.
    $mech_top_wiring = $this->getParagraphFieldValue($mapped_data['mech.top_wiring'], $existing_mech_top_wiring ?? NULL);
    // Process 'mech.package_depth'.
    $mech_packaging_depth = $this->getParagraphFieldValue($mapped_data['mech.package_depth'], $existing_mech_packaging_depth ?? NULL);
    // Process 'mech.package_height'.
    $mech_packaging_height = $this->getParagraphFieldValue($mapped_data['mech.package_height'], $existing_mech_packaging_height ?? NULL);
    // Process 'mech.package_weight'.
    $mech_packaging_weight = $this->getParagraphFieldValue($mapped_data['mech.package_weight'], $existing_mech_packaging_weight ?? NULL);
    // Process 'mech.package_width'.
    $mech_packaging_width = $this->getParagraphFieldValue($mapped_data['mech.package_width'], $existing_mech_packaging_width ?? NULL);
    // Process 'mech.product_depth'.
    $mech_product_depth = $this->getParagraphFieldValue($mapped_data['mech.product_depth'], $existing_mech_product_depth ?? NULL);
    // Process 'mech.product_height'.
    $mech_product_height = $this->getParagraphFieldValue($mapped_data['mech.product_height'], $existing_mech_product_height ?? NULL);
    // Process 'mech.product_weight'.
    $mech_product_weight = $this->getParagraphFieldValue($mapped_data['mech.product_weight'], $existing_mech_product_weight ?? NULL);
    // Process 'mech.product_width'.
    $mech_product_width = $this->getParagraphFieldValue($mapped_data['mech.product_width'], $existing_mech_product_width ?? NULL);
    ...
    $env_ip_rating = $this->setParagraphTaxonomyField('ip_rating', $mapped_data['env.ip_rating'], $existing_env_ip_rating ?? NULL);
    // Process env.operating_humidity'.
    $env_op_humanity = $this->getParagraphFieldValue($mapped_data['env.operating_humidity'], $existing_env_op_humanity ?? NULL);
    // Process env.op_temp_max_rate'.
    $env_op_temp_max_rating = $this->getParagraphFieldValue($mapped_data['env.op_temp_max_rate'], $existing_env_op_temp_max_rating ?? NULL);
    // Process env.op_temp_min_rate'.
    $env_op_temp_min_rating = $this->getParagraphFieldValue($mapped_data['env.op_temp_min_rate'], $existing_env_op_temp_min_rating ?? NULL);
    // Process env.st_temp_max_rate'.
    $env_storage_temp_max_rating = $this->getParagraphFieldValue($mapped_data['env.st_temp_max_rate'], $existing_env_storage_temp_max_rating ?? NULL);
    // Process env.st_temp_min_rate'.
    $env_storage_temp_min_rating = $this->getParagraphFieldValue($mapped_data['env.st_temp_min_rate'], $existing_env_storage_temp_min_rating ?? NULL);
  ...
  public function setParagraphTaxonomyField($vocabulary_name, $field_value, $existing_term_value) {
    if (isset($field_value)) {
      if ($field_value !== '' && $field_value !== 'NULL') {
        // Check if the vocabulary exists.
        $vocabulary = $this->entityTypeManager->getStorage('taxonomy_vocabulary')->load($vocabulary_name);
